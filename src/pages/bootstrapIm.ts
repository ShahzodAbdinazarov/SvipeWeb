...
export async function bootstrapIm(): Promise<void> {
  ...
  // mobile widths.
  // Svipe: mounts the tab bar, which on boot selects Reels (default surface on
  // mobile) and honours a ?svipeReel=<code> share deep-link. This funnel runs on
  // both a boot-signed-in start and a just-completed login, so it covers either.
